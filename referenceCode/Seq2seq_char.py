...
with open('data/letters_source2.txt', 'r', encoding='utf-8') as f:
    source_data = f.read()

with open('data/letters_target2.txt', 'r', encoding='utf-8') as f:
    target_data = f.read()
...
#dict.get(key,3) #如果获取key不在字典中就用3作为替代。
# 对字母进行转换
...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size,
    ...
    with tf.variable_scope("decode"):
        ...
        #tf.contrib.seq2seq.dynamic_calls
...
with tf.Session(graph=train_graph) as sess:
    ...
    for epoch_i in range(1, epochs+1):
        for batch_i, (targets_batch, sources_batch, targets_lengths, sources_lengths) in enumerate(
            ...
            # 3:<EOS> 2:<GO> 0:<PAD>
            print("sources_batch[0]:",sources_batch[0])
            print("targets_batch[0]:",targets_batch[0])
